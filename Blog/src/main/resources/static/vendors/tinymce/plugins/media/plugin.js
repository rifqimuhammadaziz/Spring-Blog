...
(function () {
    ...
    var global$3 = tinymce.util.Tools.resolve('tinymce.main.DomParser');
    ...
    var global$2 = tinymce.util.Tools.resolve('tinymce.main.Node');

    var global$1 = tinymce.util.Tools.resolve('tinymce.main.Serializer');
    ...
    const setup$2 = editor => {
      ...
      editor.on('ObjectResized', e => {
        ...
        if (target.getAttribute('data-mce-object')) {
          let html = target.getAttribute('data-mce-main');
          if (html) {
            html = unescape(html);
            target.setAttribute('data-mce-main', escape(updateHtml(html, {
    ...
    const createPreviewNode = (editor, node) => {
      ...
      if (name === 'iframe') {
        ...
        const sanitizedHtml = previewWrapper.attr('data-mce-main');
    ...
    const retainAttributesAndInnerHtml = (editor, sourceNode, targetNode) => {
      ...
      if (innerHtml) {
        targetNode.attr('data-mce-main', escape(innerHtml));
        targetNode.empty();
      }
    };
      ...
      editor.on('PreInit', () => {
        ...
        serializer.addAttributeFilter('data-mce-object', (nodes, name) => {
          ...
          while (i--) {
            ...
            const innerHtml = node.attr('data-mce-main');
            if (innerHtml) {
