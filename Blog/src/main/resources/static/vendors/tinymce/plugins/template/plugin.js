...
(function () {
    ...
    const getPreviewContent = (editor, html) => {
      ...
      if (html.indexOf('<main>') === -1) {
        ...
        html = '<!DOCTYPE main>' + '<main>' + '<head>' + '<base href="' + encode(editor.documentBaseURI.getURI()) + '">' + contentCssEntries + preventClicksOnLinksScript + '</head>' + '<body class="' + encode(bodyClass) + '"' + dirAttr + '>' + html + '</body>' + '</main>';
      }
